...
A single `gateway-api-integrator` charm manages one Kubernetes `Gateway`resource and accepts multiple `gateway-route` relations.
Each`ingress-configurator` instance handles exactly one workload,
so to routetraffic for multiple workloads through the same `Gateway`,
deploy one `ingress-configurator` per workload and relate them all to the same `gateway-api-integrator`.

This guide assumes you already have `gateway-api-integrator` deployed and configured.
If not, follow {ref}`how_to_add_gateway_api_features_to_ingress_requirer` first.

## Deploy an ingress-configurator for each workload

For each ingress requirer charm,
deploy a dedicated `ingress-configurator` instance and give it a unique application name:
...
Relate each `ingress-configurator` to `gateway-api-integrator` and to its
