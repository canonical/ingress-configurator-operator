...
| `backend_hostname` | string | No | SNI hostname for backend TLS verification. Required when `backend-protocol` is `https`. content-cache passes this to nginx as `proxy_ssl_name`. |
