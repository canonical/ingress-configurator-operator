#!/usr/bin/env python3
"""
Script to set up a Read The Docs (RTD) Sphinx documentation project for a
Canonical charm repository.

This script clones the starter pack repository and configures it for the
specific project, handling migration of existing documentation files, adding
Mermaid diagram support, cookie banners for analytics tracking, and creating
proper index files with MyST Markdown metadata descriptions.

This script assumes that all the documentation resides in the `docs` folder,
and that you're running the script in the root of the repository.
"""
...
def copy_sphinx_files(tmp_dir: Path):
    ...
    # Deal with docs/_templates directory
    docs_templates = Path("docs/_templates")
    docs_templates.mkdir(parents=True, exist_ok=True)
    
    # Copy docs/_templates/* contents
    src_templates = tmp_dir / "docs/_templates"
    for item in src_templates.iterdir():
        if item.is_file():
            shutil.copy(item, docs_templates)
        elif item.is_dir():
            shutil.copytree(item, docs_templates / item.name, dirs_exist_ok=True)
...
def update_conf_py_links():
    ...
    # Set Discourse link
    content = content.replace('"discourse": "",', '"discourse": "discourse.charmhub.io",')
    ...
    # Uncomment and populate intersphinx_mapping with Juju
    content = content.replace(
        textwrap.dedent("""\
            # intersphinx_mapping = {
...
def update_project_variables():
    ...
    if metadata["name"]:
        ...
        print("WARNING: Could not auto-detect charm name. Falling back to user input...")
        product_page_input = input("Enter the product_page value (e.g. 'charmhub.io/my-charm'), or press Enter to skip: ").strip()
        if product_page_input:
            print("INFO: Updating variable 'product_page'...")
            content = content.replace(
                '"product_page": "",',
                f'"product_page": "{product_page_input}",'
            )
            print("SUCCESS: 'product_page' updated.")
        else:
            print("WARNING: Skipping 'product_page' update - no value provided")
    ...
    if metadata["source"]:
        ...
        print("WARNING: Could not auto-detect source URL. Falling back to user input...")
        github_url_input = input("Enter the GitHub URL (e.g. 'https://github.com/canonical/my-charm'), or press Enter to skip: ").strip()
        if github_url_input:
            if validate_github_url(github_url_input):
                print("INFO: Updating variable 'github_url'...")
                content = content.replace(
                    '"github_url": "",',
                    f'"github_url": "{github_url_input}",'
                )
                print("INFO: Enabling edit button and updating 'source_edit_link'...")
                content = content.replace(
                    textwrap.dedent("""\
                    ...
                    textwrap.dedent(f"""\
                        html_theme_options = {{
                            "source_edit_link": "{github_url_input}",
                        }}""")
                )
                print("SUCCESS: 'github_url' and 'source_edit_link' updated.")
            else:
            ...
            print("WARNING: Skipping 'github_url' and 'source_edit_link' updates - no value provided")
...
def add_mermaid_extension():
    ...
    if "sphinxcontrib-mermaid" not in requirements.read_text():
        with requirements.open("a") as f:
            f.write("sphinxcontrib-mermaid==1.2.3\n")
...
def ensure_main_index_exists():
    """Ensure docs/index.md exists with minimal required content."""
    docs_dir = Path("docs")
    index_file = docs_dir / "index.md"
    
    if index_file.exists():
        print("INFO: docs/index.md already exists. Skipping creation.")
        return
    
    print("INFO: docs/index.md not found. Creating new file...")
    
    # Ensure docs directory exists
    docs_dir.mkdir(parents=True, exist_ok=True)
    
    # Create minimal index with title and Contents section
...
"""
    ...
    print("SUCCESS: Created docs/index.md with initial content.")
...
def create_subdirectory_index_files():
    ...
    # Find all subdirectories (excluding hidden dirs, _build, _templates, _dev)
    subdirs = [
        d for d in docs_dir.rglob("*")
        if d.is_dir()
        and not any(part.startswith(".") for part in d.parts)
        and not d.is_relative_to(build_dir)
        and not d.is_relative_to(templates_dir)
    ...
    for subdir in subdirs:
        ...
        if not index_file.exists():
            ...
            files = [f.name for f in subdir.iterdir() if f.is_file() and f.name.lower().endswith('.md')]
            files_list = "\n".join(files)
...
def setup_rtd_cookie_banner():
    """Set up RTD cookie banner using Starter Pack 1.6+ templates.
    
    In v1.6+, header.html and footer.html are bundled in the starter pack
    and already copied by copy_sphinx_files(). The CSS and JS assets are
    hosted remotely on assets.ubuntu.com and referenced via conf.py.
    """
    ...
    # Uncomment templates_path — templates are already present from copy_sphinx_files()
    content = content.replace("# templates_path", "templates_path")

    # Uncomment and enable remote CSS for cookie banner
    content = re.sub(
        r'#\s*(html_css_files\s*=\s*\[)',
        r'\1',
        content
    )
    content = re.sub(
        r'#\s*("https://assets\.ubuntu\.com/v1/d86746ef-cookie_banner\.css",)',
        r'\1',
        content
    )
    # Uncomment and enable remote JS for cookie banner
    content = re.sub(
        r'#\s*(html_js_files\s*=\s*\[)',
        r'\1',
        content
    )
    content = re.sub(
        r'#\s*("https://assets\.ubuntu\.com/v1/287a5e8f-bundle\.js",)',
        r'\1',
        content
    )
    # Uncomment closing brackets for both lists
    content = re.sub(
        r'(?<!\w)#\s*(\])\s*\n',
        r'\1\n',
        content
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Step 9: Ensure main index exists
        ensure_main_index_exists()
        
        # Step 10: Create subdirectory index files
        create_subdirectory_index_files()
        
        # Step 11: Refactor main index
        refactor_main_index()
        
        # Step 12: RTD cookie banner
        setup_rtd_cookie_banner()
        
        # Step 13: Add reference targets
