...
def test_adapter_end_to_end_routing(
    ...
    """
    juju_machine.integrate(f"{haproxy}:haproxy-route", f"{application}:haproxy-route")
    juju_machine.wait(
    ...
    session = http_session(
        dns_entries=[(MOCK_HAPROXY_HOSTNAME, str(get_unit_addresses(juju_machine, haproxy)[0]))]
    )
...
def test_adapter_http(
    ...
    addr = str(get_unit_addresses(juju_machine, haproxy)[0])
    ...
    juju_machine.config(application, {"allow-http": True})
    juju_machine.wait(
