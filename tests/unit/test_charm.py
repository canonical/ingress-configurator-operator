...
def test_config_changed_integrator_invalid_paths(monkeypatch: pytest.MonkeyPatch, context):
    """
    arrange: prepare some state with invalid paths.
    act: trigger a config changed event with invalid paths configuration.
    assert: status is blocked with the correct message.
    """
    monkeypatch.setattr(state, "get_mode", MagicMock(return_value=state.Mode.INTEGRATOR))
    charm_state = ops.testing.State(
    ...
    assert out.unit_status == ops.testing.BlockedStatus(
        "Invalid integrator configuration: paths-0"
    )


def test_config_changed_integrator_invalid_subdomains(monkeypatch: pytest.MonkeyPatch, context):
    """
    arrange: prepare some state with invalid subdomains.
    act: trigger a config changed event with invalid subdomains configuration.
    assert: status is blocked with the correct message.
    """
    monkeypatch.setattr(state, "get_mode", MagicMock(return_value=state.Mode.INTEGRATOR))
    charm_state = ops.testing.State(
    ...
    assert out.unit_status == ops.testing.BlockedStatus(
        "Invalid integrator configuration: subdomains-0"
    )
