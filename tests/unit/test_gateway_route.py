...
def test_gateway_route_config_strip_prefix_overrides_unset_relation_value(
    context_k8s: ops.testing.Context["IngressConfiguratorCharm"], mock_lightkube: "LightkubeClient"
):
    """
    arrange: requirer does not publish strip-prefix, but the operator sets strip-prefix=true.
    act: trigger config-changed.
    assert: the HTTPRoute rule carries the URLRewrite filter.
    """
    state = ops.testing.State(
        config={"hostname": "example.com", "paths": "/app1", "strip-prefix": True},
        relations=[
            ops.testing.Relation(
                endpoint="ingress",
                remote_app_data=INGRESS_REMOTE_APP_DATA,
    ...
    (single_call,) = mock_lightkube.apply.call_args_list  # type: ignore[attr-defined]
    assert single_call.args[0].spec["rules"][0]["filters"] == [STRIP_PREFIX_FILTER]


def test_gateway_route_config_strip_prefix_false_overrides_relation_true(
    context_k8s: ops.testing.Context["IngressConfiguratorCharm"], mock_lightkube: "LightkubeClient"
):
    """
    arrange: requirer publishes strip-prefix=true but the operator sets strip-prefix=false.
    act: trigger config-changed.
    assert: no filters are emitted — the explicit config wins.
    """
    state = ops.testing.State(
        config={"hostname": "example.com", "paths": "/app1", "strip-prefix": False},
        relations=[
            ops.testing.Relation(
                endpoint="ingress",
                remote_app_data={**INGRESS_REMOTE_APP_DATA, "strip-prefix": "true"},
    ...
    (single_call,) = mock_lightkube.apply.call_args_list  # type: ignore[attr-defined]
    assert "filters" not in single_call.args[0].spec["rules"][0]
